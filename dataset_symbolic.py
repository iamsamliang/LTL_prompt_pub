...
def construct_split_dataset(data_fpath, split_dpath, holdout_type, feasible_types, filter_types, perm_props, size, seed, firstn, sample_size=None):
    """
    ...
    :param size: size of each fold for type and formula holdout; for utterance holdout, proportion of the training dataset to be the holdout set.
    ...
    if holdout_type == "ltl_type":  # hold out specified pattern types
        ...
        for fold_idx, (train_indices, holdout_indices) in enumerate(kf.split(feasible_types)):
            ...
            if sample_size is not None:
                examples_per = {}
                assert sample_size > len(holdout_types)
                even_divide = sample_size // len(holdout_types)
                leftover = sample_size % len(holdout_types)

                for formula_type in holdout_types:
                    if leftover > 0:
                        examples_per[formula_type] = even_divide + 1
                        leftover -= 1
                    else:
                        examples_per[formula_type] = even_divide

                smaller_valid, smaller_valid_meta = [], []
                for pattern_type, props_str, utt, ltl in dataset:
                    props = deserialize_props_str(props_str)
                    if pattern_type in holdout_types:
                        num_per = examples_per.get(pattern_type)
                        if num_per > 0:
                            examples_per[pattern_type] -= 1
                            smaller_valid.append((utt, ltl))
                            smaller_valid_meta.append((pattern_type, props))

                dataset_name = Path(data_fpath).stem

                print("Total number of LTL formula types possible: ", len(feasible_types))
                print("Number of LTL formula types in validation set (there are no examples in the prompt of these LTL types): ", len(holdout_types))
                print("Size of type holdout validation dataset: ", len(smaller_valid))
                print()
                
                small_valid_dataset = {
                    "smaller_valid": smaller_valid, 
                    "smaller_valid_meta": smaller_valid_meta,
                    "holdout_type": holdout_type,
                    "holdout_meta": holdout_types,
                    "seed": seed,
                    "size": size,
                    "n_folds": len(feasible_types)//size,
                    "dataset_size": len(smaller_valid)
                }
                smaller_destination = f"prompt_eng/data/{sample_size}_holdout_batch1_noperm"
                os.makedirs(smaller_destination, exist_ok=True)
                smaller_destination = f"prompt_eng/data/{sample_size}_holdout_batch1_noperm/{dataset_name}_{holdout_type}_{size}_{seed}_fold{fold_idx}.pkl"
                save_to_file(small_valid_dataset, smaller_destination)
            else:
                holdout_types = [feasible_types[idx] for idx in holdout_indices]
                ...
                print("Total number of LTL formula types possible: ", len(feasible_types))
                print("Number of LTL formula types in validation set (there are no examples in the prompt of these LTL types): ", len(holdout_types))
                print("Size of type holdout training dataset: ", len(train_iter))
                print("Size of type holdout validation dataset: ", len(valid_iter))
                print()
        ...
        for fold_idx, (train_indices, holdout_indices) in enumerate(kf.split(all_formulas)):
            ...
            if sample_size is not None:
                examples_per = {}
                assert sample_size > len(holdout_formulas)
                even_divide = sample_size // len(holdout_formulas)
                leftover = sample_size % len(holdout_formulas)

                for formula_instance in holdout_formulas:
                    if leftover > 0:
                        examples_per[formula_instance] = even_divide + 1
                        leftover -= 1
                    else:
                        examples_per[formula_instance] = even_divide
                ...
                for pattern_type, props_str, utt, ltl in dataset:
                    ...
                    if formula in holdout_formulas:
                        num_per = examples_per.get(formula)
                        if num_per > 0:
                            examples_per[formula] -= 1
                            smaller_valid.append((utt, ltl))
                            smaller_valid_meta.append((pattern_type, props))

                print("Number of unique LTL formula type with number of propositions: ", len(all_formulas))
                print("Number of LTL formula types with different number of propositions in validation set (LTL formula types with x number of propositions in the validation set are not in the prompt examples. But there can be overlap between LTL formula types. Just not LTL formula types with the number of examples.): ", len(holdout_formulas))
                print("Size of formula holdout validation dataset: ", len(smaller_valid))
                print()
                ...
                small_valid_dataset = {
                    ...
                    "smaller_valid_meta": smaller_valid_meta,
                    "holdout_type": holdout_type,
                    "holdout_meta": holdout_formulas,
                    "seed": seed,
                    "size": size,
                    "n_folds": len(all_formulas)//size,
                    "dataset_size": len(smaller_valid)
                }
                smaller_folder = f"prompt_eng/data/{sample_size}_holdout_batch1_noperm"
                os.makedirs(smaller_folder, exist_ok=True)
                smaller_destination = f"prompt_eng/data/{sample_size}_holdout_batch1_noperm/{dataset_name}_{holdout_type}_{size}_{seed}_fold{fold_idx}.pkl"
                save_to_file(small_valid_dataset, smaller_destination)
            else:
                ...
                print("Number of unique LTL formula type with number of propositions: ", len(all_formulas))
                print("Number of LTL formula types with different number of propositions in validation set (LTL formula types with x number of propositions in the validation set are not in the prompt examples. But there can be overlap between LTL formula types. Just not LTL formula types with the number of examples.): ", len(holdout_formulas))
                print("Size of formula holdout training dataset: ", len(train_iter))
                print("Size of formula holdout validation dataset: ", len(valid_iter))
                print()
                save_split_dataset(split_fpath, train_iter, train_meta, valid_iter, valid_meta,
        ...
        if sample_size is not None:
            random.seed(seed)
            examples_per = {}
            assert sample_size > len(meta2data)
            even_divide = sample_size // len(meta2data)
            leftover = sample_size % len(meta2data)

            for formula_instance in meta2data.keys():
                if leftover > 0:
                    examples_per[formula_instance] = even_divide + 1
                    leftover -= 1
                else:
            ...
            for (pattern_type, nprops), data in meta2data.items():
                ...
                num_per = examples_per.get((pattern_type, nprops))
                sampled_elements = random.sample(valid_dataset, num_per)
                meta_data_list = [(pattern_type, PROPS[:nprops])] * num_per
                smaller_valid.extend(sampled_elements)
                smaller_valid_meta.extend(meta_data_list)
                unique_instances.add((pattern_type, nprops))

            print()
            print("Number of unique LTL formula type with number of propositions: ", len(meta2data))
            print("Number of unique LTL formula type with number of propositions in validation set: ", len(unique_instances))
            print("Size of utterance holdout validation dataset: ", len(smaller_valid))
            print()
            ...
            small_valid_dataset = {
                ...
                "smaller_valid_meta": smaller_valid_meta,
                "holdout_type": holdout_type,
                "holdout_meta": list(meta2data.keys()),
                "seed": seed,
                "size": size,
                "dataset_size": len(smaller_valid)
            }
            smaller_folder = f"prompt_eng/data/{sample_size}_holdout_batch1_noperm"
            os.makedirs(smaller_folder, exist_ok=True)
            smaller_destination = f"prompt_eng/data/{sample_size}_holdout_batch1_noperm/{dataset_name}_{holdout_type}_{size}_{seed}.pkl"
            save_to_file(small_valid_dataset, smaller_destination)
        else:
            ...
            print("Number of unique LTL formula type with number of propositions: ", len(meta2data))
            print("Number of unique LTL formula type with number of propositions in validation set: ", len(unique_instances))
            print("Size of utterance holdout training dataset: ", len(train_iter))
            print("Size of utterance holdout validation dataset: ", len(valid_iter))
            print()
            save_split_dataset(split_fpath, train_iter, train_meta, valid_iter, valid_meta,
...
def generate_prompts_from_split_dataset(split_fpath, prompt_dpath, nexamples, seed, is_cot):
    """
    ...
    prompt = "Your task is to translate English instructions into linear temporal logic (LTL) formulas.\n\n"
    for (pattern_type, nprop), data in meta2data.items():
        random.seed(seed)
        examples = random.sample(data, nexamples)
        for utt, ltl in examples:
            prompt += f"Q: What is \"{utt}\" in LTL?\nA: {ltl}\n\n"
            # print(f"{pattern_type} | {nprop}\n{utt}\n{ltl}\n")
    prompt += "Q:"

    split_dataset_name = Path(split_fpath).stem
    if is_cot:
        prompt_fpath = f"{prompt_dpath}/CoT_prompt_nexamples{nexamples}_{split_dataset_name}.txt"
    else:
...
if __name__ == "__main__":
    # python dataset_symbolic.py --nexamples=1
    ...
    parser.add_argument("--perm", action="store_true", help="True if permute props after train, test split.") # false
    parser.add_argument("--update", action="store_true", help="True if update existing symbolic dataset w/ new responses.") # false
    parser.add_argument("--merge", action="store_true", help="True if merge Google form responses from batches.") # false
    parser.add_argument("--remove_pun", action="store_true", help="True if remove punctuations.") # false
    parser.add_argument("--seeds_split", action="store", type=int, nargs="+", default=[484, 123, 324], help="1 or more random seeds for train, test split.")
    ...
    parser.add_argument("--seed_prompt", type=int, default=123, help="random seed for choosing prompt examples.")
    ...
    symbolic_fpath = "data/symbolic_batch1_noperm.csv"
    split_dpath = "prompt_eng/data/holdout_batch1_noperm"
    prompt_dpath = "prompt_eng/data/prompts_batch1_noperm"
    os.makedirs(split_dpath, exist_ok=True)
    os.makedirs(prompt_dpath, exist_ok=True)

    # used to create smaller datasets
    sample_size = None

    # construct utterance holdout split
    for seed in seeds:
        construct_split_dataset(symbolic_fpath, split_dpath, "utt", FEASIBLE_TYPES, FILTER_TYPES, args.perm, size=0.92, seed=seed, firstn=args.firstn, sample_size=sample_size)
    ...
    # Construct train, test split for LTL formula and type holdout;
    # construct_split_dataset(symbolic_fpath, split_dpath, "ltl_type", FEASIBLE_TYPES, FILTER_TYPES, args.perm, size=2, seed=123, firstn=args.firstn, sample_size=sample_size)
    # construct_split_dataset(symbolic_fpath, split_dpath, "ltl_formula", FEASIBLE_TYPES, FILTER_TYPES, args.perm, size=4, seed=123, firstn=args.firstn, sample_size=sample_size)

    # Generate plain or CoT prompts for off-the-shelf GPT-3
    is_cot = False
    ...
    if sample_size is None:
        ...
        for split_fpath in split_fpaths:
            for n in nexamples:
                generate_prompts_from_split_dataset(split_fpath, prompt_dpath, n, args.seed_prompt, is_cot)
